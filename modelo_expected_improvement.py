#Kriging universal con expected improvement
...
S = np.array([[puntos_x[i],puntos_y[i],puntos_z[i]] for i in range(n_agentes)])
...
def Jres(x,xi,xd,uk,alpha,beta,b,phi_max):
    ...
    if sigma > 0:
        z = (phi-phi_max)/sigma
        EI = (phi-phi_max)*norm.cdf(z)+sigma*norm.pdf(z)
    else:
        EI = 0
    ...
    return alpha*(x-xi)@(x-xi)-beta*j+1e10*(EI<=5e-1)+1e10*(dist<=5e-1)

#Área condicionada
def EI(mu,sigma,phi_max):
    Z = np.zeros_like(mu)
    Z[sigma>0] = (mu[sigma>0]-phi_max)/sigma[sigma>0]  
    EI = np.zeros_like(mu)
    EI[sigma>0] = (mu[sigma>0]-phi_max)*norm.cdf(Z[sigma>0])+sigma[sigma>0]*norm.pdf(Z[sigma>0])
    return EI
...
b = 1
alpha = 1e-1  #alpha modificado de 1e-2 a 1e-1
beta = 5/n_agentes  #beta modificado de 1/n_agentes a 5/n_agentes
...
#Para pintar la zona donde cond<cota
cota = 2e-1
D = np.ma.masked_where(~(EI(campo_estimado,(varianza_estimada**0.5),phi_max)<cota),campo_estimado)

#Ley de control
M = 1.0
k1 = 1
k2 = 1  #k2 modificada de 2 a 1
k3 = 200  #k3 modificada de 100 a 200
q = 0.1
paso = 0.01
t0 = 0
t_max = 100000
...
cond = EI(campo_estimado,(varianza_estimada**0.5),phi_max)
while (np.max(cond)>cota) & (it<200):
